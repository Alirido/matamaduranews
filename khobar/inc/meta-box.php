...
class Video_Meta_Box {
	...
	public function render_metabox( $post ) {
		...
		echo '	<tr>';
		echo '		<th><label for="youtube_url" class="youtube_url_label">https://youtube.com/</label></th>';
		echo '		<td>';
		echo '			<input type="text" id="youtube_url" placeholder="9rlszUK1UcQ" name="youtube_url" class="youtube_url_field" placeholder="' . esc_attr__( '', 'text_domain' ) . '" value="' . esc_attr__( $youtube_url ) . '">';
		echo '		</td>';
		echo '	</tr>';
/*
		echo '	<tr>';
...
*/
	...
	public function save_metabox( $post_id, $post ) {
		...
		// Sanitize user input.
		$car_new_year = isset( $_POST[ 'youtube_url' ] ) ? sanitize_text_field( $_POST[ 'youtube_url' ] ) : '';
		...
		update_post_meta( $post_id, 'youtube_url', $car_new_year );
