...
class tuxGoogleTypography {
	...
	function __construct() {
		...
		if ( is_admin() ){
			...
			if ( TUX_TYPOGRAPHY_GENERATE_PREVIEWS )
	...
	public function has_preview_image($font) {
		...
		if (!$exists) {
			...
			$option = get_option('tux_typography_missing_previews', false);
			if (empty($option)) {
				$option = array();
			...
			if (!in_array($font, $option)) {
				...
				update_option('tux_typography_missing_previews', $new);
			}
		}
		return $exists;
	}
	...
	public static function &init() {
		...
		if ( !$instance ) {
			$instance = new tuxGoogleTypography();
		}
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', 'tuxtheme');
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'tuxtheme').'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'tuxtheme').'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'tuxtheme').'</a></p>';
		$customizing_title 		= __('Customizing','tuxtheme');
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'tuxtheme').'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'tuxtheme').'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'tuxtheme').'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'tuxtheme').'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'tuxtheme').'</li><li><b>'.__('Font Variant', 'tuxtheme').'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'tuxtheme').'</li><li><b>'.__('Font Size', 'tuxtheme').'</b> - '.__('The size you would like this font to be.', 'tuxtheme').'</li><li><b>'.__('Font Color', 'tuxtheme').'</b> - '.__('The color you\'d like to use for this font.', 'tuxtheme').'</li><li><b>'.__('CSS Selectors', 'tuxtheme').'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'tuxtheme').'</li></ul>';
		$selectors_title 			= __('CSS Selectors','tuxtheme');
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'tuxtheme') . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'tuxtheme') . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'tuxtheme').':</b> '.__('#selector', 'tuxtheme').'</li><li><b>'.__('Classes:', 'tuxtheme').':</b> '.__('.selector', 'tuxtheme').'</li><li><b>'.__('HTML Tags', 'tuxtheme').':</b> '.__('span', 'tuxtheme').'</ul>';      
		$selectors_content	 .= '<p><b>'.__('Example', 'tuxtheme').':</b> '.__('#selector span.date', 'tuxtheme').'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'tuxtheme' ) );
		}
		return $links;
	}
	...
	function build_frontend() {
		...
		$collections = get_option(TUX_TYPOGRAPHY_COLLECTIONS_OPT);
		...
		if($collections) {
            ...
            if ($uses_google_fonts) {
                $subsets = (empty($tux_options['tux_typography_sets']) ? array('latin') : $tux_options['tux_typography_sets']);
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', 'tuxtheme');
		$loading							= __('Loading Your Collections', 'tuxtheme');
		$add_new        			= __('Add New Collection', 'tuxtheme');
		$reset        				= __('Reset Collections', 'tuxtheme');
		$preview_text   			= __('Type in some text to preview...', 'tuxtheme');
		$collection_title 			= __('New Collection', 'tuxtheme');
        //$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'tuxtheme');
		//$font_family_title  	= __('Font family...', 'tuxtheme');
        $std_font_family_title  	= __('Standard Fonts', 'tuxtheme');
        $google_font_family_title  	= __('Google Fonts', 'tuxtheme');
		$font_variant_title 	= __('Variant...', 'tuxtheme');
		$font_size_title 			= __('Size...', 'tuxtheme');
        $backup_font_title          = __('Backup font:', 'tuxtheme');
		$css_selectors_title	= __('CSS Selectors:', 'tuxtheme');
        $additional_css_title	= __('Additional CSS:', 'tuxtheme');
		$delete_button_text   = __('Delete', 'tuxtheme');
		$save_button_text			= __('Save', 'tuxtheme');
		
		$welcome_title 				= __('Theme Typography', 'tuxtheme');
		$welcome_subtitle			= __('Get started in 3 steps.', 'tuxtheme');//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'tuxtheme').'</a>';
		$step_1_title 				= __('1. Pick A Font', 'tuxtheme');
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', 'tuxtheme');
		$step_2_title 				= __('2. Customize It', 'tuxtheme');
		$step_2_desc					= __('Pick a size, variant, color and more.', 'tuxtheme');
		$step_3_title 				= __('3. Attach It', 'tuxtheme');
		$step_3_desc					= __('Attach your font to any CSS selector(s).', 'tuxtheme');
		$year									= date("Y");
        $themename = TUX_THEME_NAME;
        
        $character_sets_title = __('Character sets', 'tuxtheme');
        $character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'tuxtheme');
		
        if ( empty($tux_options['tux_typography_sets']) || ! is_array( $tux_options['tux_typography_sets'] ) )
            $tux_options['tux_typography_sets'] = array('latin'); // default

        $character_sets = array(
            'latin' => __('Latin', 'tuxtheme'),
            'latin-ext' => __('Latin Extended', 'tuxtheme'),
            'cyrillic' => __('Cyrillic', 'tuxtheme'),
            'cyrillic-ext' => __('Cyrillic Extended', 'tuxtheme'),
            'greek' => __('Greek', 'tuxtheme'),
            'greek-ext' => __('Greek Extended', 'tuxtheme'),
            'vietnamese' => __('Vietnamese', 'tuxtheme'),
            'khmer' => __('Khmer', 'tuxtheme'),
            'devanagari' => __('Devanagari', 'tuxtheme'),
        );
		...
		if(get_option(TUX_TYPOGRAPHY_DEFAULT_OPT)) {
					...
					<div class="collection">
						...
						<div class="font_options">
							<div class="left_col">
								...
								<select class="font_size tux-font-size">
...
EOT;
        ...
        foreach ($character_sets as $val => $name) {
            $typography_settings .= '<label for="tux_typography_sets_'.$val.'">
                                <input type="checkbox" id="tux_typography_sets_'.$val.'" name="'.$themename.'[tux_typography_sets][]" regular-text value="'.$val.'" '.checked( in_array( $val, $tux_options['tux_typography_sets'] ), true, false ).'/> 
                                '.$name.' ('.$val.')
                            </label><br/>';
        }
	...
	function ajax_save_user_fonts() {
		...
		$collections = update_option(TUX_TYPOGRAPHY_COLLECTIONS_OPT, $collections);
	...
	function generate_font_previews() {
		...
		foreach ($option as $i => $font) {
			...
			update_option('tux_typography_missing_previews', array_diff($option, array($font)));
			$c++;
		}
	}
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
	            'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'tuxtheme'),
	            'delete_confirm' => __("Are you sure you want to delete this collection?", 'tuxtheme')
	        ));
 ...
 *
 */
function tux_register_typography($collections) {
	...
	if(!get_option(TUX_TYPOGRAPHY_DEFAULT_OPT)) {
		...
		update_option(TUX_TYPOGRAPHY_DEFAULT_OPT, true);
		update_option(TUX_TYPOGRAPHY_COLLECTIONS_OPT, $defaults);
