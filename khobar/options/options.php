...
define('TUX_TYPOGRAPHY_DEFAULT_OPT', TUX_THEME_NAME.'_typography_default');
define('TUX_TYPOGRAPHY_COLLECTIONS_OPT', TUX_THEME_NAME.'_typography_collections');
$tux_typography_generate_previews = ( extension_loaded('gd') && function_exists('gd_info') && function_exists('imagettftext') );
define('TUX_TYPOGRAPHY_GENERATE_PREVIEWS', $tux_typography_generate_previews);
...
class NHP_Options{
	...
	*/
	function __construct($sections = array(), $args = array(), $extra_tabs = array()){
		...
		$defaults['menu_title'] = __('Options', 'tuxtheme');
		$defaults['page_icon'] = 'icon-themes';
		$defaults['page_title'] = __('Options', 'tuxtheme');
        ...
        $defaults['show_translate'] = false;
        $defaults['show_child_theme_opts'] = false;
		$defaults['dev_mode'] = true;
		$defaults['stylesheet_override'] = false;
		
		$defaults['footer_credit'] = __('', 'tuxtheme');
		
		$defaults['help_tabs'] = array();
		$defaults['help_sidebar'] = __('', 'tuxtheme');
        ...
        add_action('wp_ajax_tux_translation_panel',array(&$this,'ajax_tux_translation_panel'));
        add_action('wp_ajax_tux_save_translation',array(&$this,'ajax_tux_save_translation'));

        //child theme creation
        add_action('wp_ajax_tux_child_theme', array(&$this,'tux_child_theme'));
        add_action('wp_ajax_tux_list_child_themes', array(&$this,'tux_list_child_themes'));
	...
	*/
	function _enqueue(){
        ...
        wp_localize_script( 'nhp-opts-js', 'nhpopts', array(
            ...
            'reset_confirm' => __('Are you sure you want to reset ALL options to default (except theme translation)?', 'tuxtheme'),
            'reset_translations_confirm' => __('Are you sure you want to reset translations? You will lose all translations.', 'tuxtheme'),
            'import_confirm' => __('Are you sure you want to import options? All current options will be lost.', 'tuxtheme'),
            'leave_page_confirm' => __('Settings have changed, you should save them! Are you sure you want to leave this page?', 'tuxtheme'),
            'child_theme_name_empty' => __('Please enter desired child theme name.', 'tuxtheme')
        ) );
	...
	*/
	function _validate_options($plugin_options){
        ...
        if (!empty($plugin_options['google_typography_collections'])) {
            ...
            update_option(TUX_TYPOGRAPHY_COLLECTIONS_OPT, $collections);
        }
        ...
        //     update_option('tux_translations_'.TUX_THEME_NAME, $plugin_options['translations']);
        // }
		...
		if(!empty($plugin_options['import'])){
			...
			if(is_array($imported_options) && isset($imported_options['nhp-opts-backup']) && $imported_options['nhp-opts-backup'] == '1'){
                ...
                if (!empty($imported_options['google_typography_collections'])) {
                    update_option(TUX_TYPOGRAPHY_COLLECTIONS_OPT, $imported_options['google_typography_collections']);
                }
                unset($imported_options['google_typography_collections']);
                
				// same for Translations
                if (!empty($imported_options['translations'])) {
                    update_option('tux_translations', $imported_options['translations']);
                }
        ...
        if(!empty($plugin_options['reset_translations'])) {
            ...
            delete_option('tux_translations_'.$this->args['opt_name']);
		...
		if(!empty($plugin_options['defaults'])){
            ...
            // Fonts
            delete_option(TUX_TYPOGRAPHY_DEFAULT_OPT);
            delete_option(TUX_TYPOGRAPHY_COLLECTIONS_OPT);
            
            // don't reset tux_translations
	...
	function _options_page_html(){
		...
		echo '<div class="wrap">';
			...
			echo '<form method="post" action="options.php" enctype="multipart/form-data" id="nhp-opts-form-wrapper">';
				...
				echo '<a href="http://tuxtheme.com" id="optionpanellogo" class="logo" target="_blank"><img src="'.$this->url.'img/optionpanellogo.png" /></a>';
echo '<span class="headtext">'.TUX_THEME_NAME.' V.'.TUX_PRODUCT_VERSION.'</span>';

					echo '<a href="'.TUX_SUP.'" class="docsupport" target="_blank"><i class="fa fa-medkit"></i> Support</a>';
				...
				echo '</div>';
					...
					if(isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' && get_transient('nhp-opts-saved') == '1'){
						if(isset($this->options['imported']) && $this->options['imported'] == 1){
							echo '<div id="nhp-opts-imported">'.__('<strong>Settings Imported!</strong>', 'tuxtheme').'</div>';
						}else{
							echo '<div id="nhp-opts-save">'.__('<strong>Settings Saved!</strong>', 'tuxtheme').'</div>';
						}
						delete_transient('nhp-opts-saved');
					}
					echo '<div id="nhp-opts-save-warn">'.__('<strong>Settings have changed, you should save them!</strong>', 'tuxtheme').'</div>';
					echo '<div id="nhp-opts-field-errors">'.__('<strong><span></span> error(s) were found!</strong>', 'tuxtheme').'</div>';
					
					echo '<div id="nhp-opts-field-warnings">'.__('<strong><span></span> warning(s) were found!</strong>', 'tuxtheme').'</div>';
					...
					echo '<ul id="nhp-opts-group-menu">';
						...
						if(true === $this->args['show_typography']){
							echo '<li id="typography_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="typography_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="typography_default"><i class="fa fa-text-width"></i> '.'<span class="section_title">'.__('Typography', 'tuxtheme').'</span></a>';
							echo '</li>';
                        ...
                        if(true === $this->args['show_translate']){
							echo '<li id="translation_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="translation_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="translation_default"><i class="fa fa-flag-o"></i> '.'<span class="section_title">'.__('Translate', 'tuxtheme').'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['show_import_export']){
							echo '<li id="import_export_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="nhp-opts-group-tab-link-a" data-rel="import_export_default"><i class="fa fa-sign-in"></i> '.'<span class="section_title">'.__('Import / Export', 'tuxtheme').'</span></a>';
							echo '</li>';
						...
						if(true === $this->args['dev_mode']){
							echo '<li id="dev_mode_default_section_group_li" class="nhp-opts-group-tab-link-li">';
									echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="nhp-opts-group-tab-link-a custom-tab" data-rel="dev_mode_default"><img src="'.$this->url.'img/glyphicons/glyphicons_195_circle_info.png" /> '.__('Dev Mode Info', 'tuxtheme').'</a>';
							echo '</li>';
						}//if
				...
				echo '<div id="nhp-opts-main">';
					...
					if(true === $this->args['show_import_export']){
						echo '<div id="import_export_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Import / Export Options', 'tuxtheme').'</h3>';
                            
							// presets
                            if (!empty($this->args['presets'])) {
                                echo '<h4>'.__('Preset Options', 'tuxtheme').'</h4>';
                                ...
                                foreach ($this->args['presets'] as $preset) {
                                    echo '<div class="preset">';
                                        ...
                                        if ( ! empty( $preset['demo'] ) )
                                            echo '<a href="'.$preset['demo'].'" target="_blank" class="button button-secondary demo-button">'.__('Demo', 'tuxtheme').'</a>';
                                        
                                        echo '<a href="#" class="button button-primary activate-button">'.__('Import', 'tuxtheme').'</a>';
                            ...
                            echo '<h4>'.__('Import Options', 'tuxtheme').'</h4>';
							...
							echo '<div id="nhp-opts-import-code-wrapper">';
								...
								echo '<div class="nhp-opts-section-desc">';
									...
									echo '<p class="description" id="import-code-description">'.apply_filters('nhp-opts-import-file-description',__('Insert your backup code below and hit Import to restore your site options from a backup.', 'tuxtheme')).'</p>';
                                ...
                                echo '<div class="nhp-opts-field-wrapper">';
                                    ...
                                    echo '<input type="submit" id="nhp-opts-import" name="'.$this->args['opt_name'].'[import]" class="button-primary" value="'.__('Import', 'tuxtheme').'">';
							...
							if( true === $this->args['show_child_theme_opts'] && !is_child_theme() ) {
								...
								echo '<h4>'.__('Child Theme', 'tuxtheme').'</h4>';
								echo '<div class="nhp-opts-section-desc">';
									echo '<p class="description">'. __('To create child theme for current theme, enter desired name and hit "Create Child Theme" button', 'tuxtheme').'</p>';
								echo '</div>';

								echo '<div class="nhp-opts-field-wrapper">';
									//echo '<label for="nhp-opts-child-name">'.__( 'Child theme name', 'tuxtheme' ).'</label><br />';
									echo '<input type="text" id="nhp-opts-child-name" name="nhp-opts-child-name" value="" />';
									echo '<input type="submit" id="nhp-opts-child-button" name="nhp-opts-child-button" class="button-primary" value="'.__('Create Child Theme', 'tuxtheme').'">';
								echo '</div>';

								echo '<div id="child-theme-list-wrap">';
									$this->tux_list_child_themes();
								echo '</div>';
							}
                    ...
                    if(true === $this->args['show_typography']){
						echo '<div id="typography_default_section_group'.'" class="nhp-opts-group-tab">';
							//echo '<h3>'.__('Theme Typography', 'tuxtheme').'</h3>';
							
							$typography = new tuxGoogleTypography();
					...
					if(true === $this->args['show_translate']){
						echo '<div id="translation_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Theme Translation Panel', 'tuxtheme').'</h3>';
                            echo '<div class="nhp-opts-section-desc">';
								echo '<p class="description">'. __('The theme translation panel provides an easy way to translate text appearing on your site, without the hassle of handling .po .mo files. Translations are saved instantly after typing in.', 'tuxtheme').'</p>';
                                echo '<p id="nhp-opts-reset-translations-action"><input type="submit" id="nhp-opts-reset-translations" name="'.$this->args['opt_name'].'[reset_translations]" class="button-primary" value="'.__('Reset translations', 'tuxtheme').'"></p>';
                            echo '</div>';
                            ...
                            echo '<div class="nhp-opts-field-wrapper">';
                                echo '<p><label for="nhp-opts-translate"><input type="checkbox" name="'.$this->args['opt_name'].'[translate]" id="nhp-opts-translate" value="1" '.checked($translate_enabled, true, false).' />'.__('Enable translation panel', 'tuxtheme').'</label></p>';
                            echo '</div>';
                            
                            echo '<div class="nhp-opts-field-wrapper" id="translate_search_wrapper">';
                                echo '<p><input type="text" val="" id="translate_search" placeholder="'.__('Search Translations', 'tuxtheme').'" />';
                            echo '</div>';
					...
					if(true === $this->args['dev_mode']){
						echo '<div id="dev_mode_default_section_group'.'" class="nhp-opts-group-tab">';
							echo '<h3>'.__('Dev Mode Info', 'tuxtheme').'</h3>';
							echo '<div class="nhp-opts-section-desc">';
				...
				echo '<div id="nhp-opts-footer">';
					...
					echo '<input type="submit" name="'.$this->args['opt_name'].'[defaults]" value="'.__('Reset to Defaults', 'tuxtheme').'" class="button-secondary" />';
					echo '<input type="submit" name="save" id="savechanges" value="'.__('Save Changes', 'tuxtheme').'" class="button-primary" disabled="disabled" />';
					echo '<div class="clear"></div><!--clearfix-->';
				echo '</div>';
	...
	*/
    ...
    function ajax_tux_translation_panel() {
        $poparser = new PoParser();
        $tux_translations = get_option('tux_translations_'.TUX_THEME_NAME);//$this->options['translations'];
        ...
        if ($search_query) {
            foreach ($entries as $string_id => $object) {
                ...
                $value = (empty($tux_translations[$message]) ? '' : $tux_translations[$message]);
        ...
        $this->tux_translation_pagination($number, $strings_per_page, $page);
        ...
        foreach ($entries as $string_id => $object) {
            ...
            if (!empty($tux_translations[$message]))
                $number_translated++;
            ...
            if ($i > ($page-1)*$strings_per_page && $i <= $page*$strings_per_page) {
                ...
                $form .= '<textarea id="translate-string-'.$i.'" data-id="'._wp_specialchars( $message, ENT_QUOTES, false, true ).'" class="tux_translate_textarea">';
        ...
        echo '<div class="translation_info">'.sprintf(__('Translated <span class="translated">%1$d</span> strings out of <span class="total">%2$d</span> <span class="percent">(%3$.2f%%)</span>', 'tuxtheme'), $number_translated, $number, $percent).'</div>';
        $this->tux_translation_pagination($number, $strings_per_page, $page);
        
        exit; // required for AJAX in WP
    }
    
    function tux_translation_pagination( $items_number, $items_per_page, $current = 1 ) {
        $max_page = ceil($items_number / $items_per_page);
        echo '<div class="tux_translation_pagination">';
    ...
    function ajax_tux_save_translation() {
        ...
        $translations = get_option('tux_translations_'.TUX_THEME_NAME);
        $translations[$id] = $val;
        update_option('tux_translations_'.TUX_THEME_NAME, $translations);
        echo 1;
        
        exit;
    }

    function tux_child_theme() {
...
EOF;
			...
			//add_action('admin_notice', array(&$this,'tux_filesystem_notice'));
		}
	}

	function tux_filesystem_notice() {
		echo '<div class="updated"><p>You do not have permission to do this.</p></div>';
	}

	function tux_get_child_themes() {
		$child_themes = array();
	...
	function tux_list_child_themes() {
		$child_themes_arr = $this->tux_get_child_themes();

		echo '<div class="nhp-opts-field-wrapper">';
		if ( !empty( $child_themes_arr ) ) {
			echo '<p class="description">' . __('Existing child themes for current theme:', 'tuxtheme' ) . '</p>';
			...
			_e('No child themes found for current theme', 'tuxtheme' );
		}
		echo '</div>';
